...
let RoutableTileFetcherRaw = class RoutableTileFetcherRaw {
    constructor(pathfinderProvider) {
    ...
    async get(url) {
        ...
        if (response.status === 200 && responseText) {
            ...
            return new tile_1.RoutableTile(url, new Set(), new Set());
        }
    }
    ...
    createNode(blob) {
        const id = blob["@id"];
        const node = new node_1.RoutableTileNode(id);
    ...
    createWay(blob) {
        const id = blob["@id"];
        const way = new way_1.RoutableTileWay(id);
...
RoutableTileFetcherRaw = __decorate([
    ...
    __metadata("design:paramtypes", [PathfinderProvider_1.default])
